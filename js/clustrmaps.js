  ...
  if ( 'clustrm_jq' in window ) {
      ...
      loadScript("https://code.jquery.com/jquery-1.12.4.min.js", function() {
          ...
          main();
                              ...
                              </elementslength;></div></div></s[1].y)return></s[1].x&&e></i[1].y)return></i[1].x&&r></l.length;a++)if(s=l[a],i=s.bbox,n=t-(this.transx*this.scale+s.left*this.scale),r=e-(this.transy*this.scale+s.top*this.scale),n=n></i.length;n++)this.regions[i[n]]&&(a=this.regions[i[n]].element.shape.getbbox(),a&&("undefined"==typeof></this.params.zoommin*this.basescale&&(t=this.params.zoommin*this.basescale),"undefined"!=typeof></div></div></div></a&&(this.transx=a),this.canvas.applytransformparams(this.scale,this.transx,this.transy),this.markers){this.container.find("g:eq(2)").css("visibility","hidden");var></s&&(this.transy=s),this.transx></this.series[t].length;e++)this.series[t][e].clear();this.scale=this.basescale,this.transx=this.basetransx,this.transy=this.basetransy,this.applytransform()},applytransform:function(){var></div></div></div></div></div></div></i.length;t++){switch(e=jvm.$("<div></div></div></=s[1];)e=t==s[0]?this.clearminvalue:t==s[1]?this.clearmaxvalue:t,o.push({label:t,value:this.getvalue(e)}),t+=n;return></t.length;e++)a+=math.round(t[e])*math.pow(256,t.length-e-1);return></t.length;e++)this.scale[e]=[t[e]]},setnormalizefunction:function(t){"polynomial"===t?this.normalize=function(t){return></"+t+'></rvml:"+t+'></arguments.length;a++){t=arguments[a];for(e></t.length;e++)t[e]<a&&(a=t[e]);else>